...
import { weekDay } from "@/common/utils.js";
import { MIN_DATE, MAX_DATE, ERR_MES } from "@/common/constant.js";
import Skeleton from "taro-skeleton";
import { flightListReq } from "@/common/api.js";
...
export default class List extends PureComponent<any, any> {
  ...
  getList = () => {
    ...
    flightListReq({
      ...
      .then((res) => {
        const companyArr = res.result?.map((item) => item.airportName);
        this.setState({
    ...
    this.setState(
      ...
      () => {
        const res = this.initFlightList.filter(
          (item) => item.airportName === flightCompanyList[value]
        );
        this.setState({
          flightList: res,
          scrollTop: 0,
      ...
      <Block key={flight.id}>
        ...
        <View className="list-item" onClick={() => this.onFlightClick(flight)}>
          ...
          <View className="air-info">
            ...
            <Text className="company-name">{airportName}</Text>
          </View>
        </View>
      </Block>
    );
